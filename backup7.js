    ...
    for (var j = 1; j < rows.length; j++) {
      ...
      if (currentRow[2].trim() == "" && currentRow[1].trim() == "") {
        ...
        if (currentRow[2].trim() != "") {
          ...
          var cardExists = getCards(listId, currentRow[2], currentRow[3], dueDate, currentRow[7]);
          //var updateCard = updateTrelloCard(currentRow[2]);

          if (cardExists)
          {
             //updateTrelloAttachment(cardExists.id,currentRow[8]);
             //updateTrelloLabels(cardExists.id,currentRow[6],existingLabels);
          }
          else
          {
            // post
...
function updateTrelloCard(cardId, cardName, cardDesc, cardDue, members){
...
// update card attachments - not finished

function updateTrelloAttachment(cardID, attachment){
  if (attachment == "") {
    return;
  }  
  var attachments = attachment.split(",");
  for (var i= 0; i< attachments.length;i++) {
    ...
    var url = constructTrelloURL("cards/"+ cardID + "/attachments");
    UrlFetchApp.fetch(url, {"method": "put", "url":attachments[i]});       
  }
  return;
}
...
function getCards(listId, cardName, cardDesc, cardDue, cardMembers){
  ...
  var values = Utilities.jsonParse(resp.getContentText());
    ...
    for (var i=values.length-1;i>=0;i--) {
      ...
      {
        cardId = values[i].id;
        var updateCard = updateTrelloCard(cardId, cardName, cardDesc, cardDue, cardMembers);
        var cardExists = true;
      }
    }
  return cardExists;
