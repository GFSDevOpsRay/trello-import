    ...
    for (var j = 1; j < rows.length; j++) {
      ...
      if (currentRow[2].trim() == "" && currentRow[1].trim() == "") {
        ...
        if (currentRow[2].trim() != "") {
          ...
          var cardExists = getCards(listId, currentRow[2], currentRow[3], dueDate);
...
function updateTrelloCard(cardId, cardName, cardDesc, cardDue){
...
function getCards(listId, cardName, cardDesc, cardDue){
  ...
  var values = Utilities.jsonParse(resp.getContentText());
    ...
    for (var i=values.length-1;i>=0;i--) {
      ...
      {
        cardId = values[i].id;
        var updateCard = updateTrelloCard(cardId, cardName, cardDesc, cardDue);
        var cardExists = true;
      }
    }
  return cardExists;
