    ...
    for (var j = 1; j < rows.length; j++) {
      ...
      if (currentRow[2].trim() == "" && currentRow[1].trim() == "") {
        ...
        if (currentRow[2].trim() != "") {
          ...
          var cardExists = getCards(currentRow[2], currentRow[3], currentRow[4], listId, dueDate, currentRow[7], currentRow[6]);
...
function updateTrelloCard(cardId, cardName, cardDesc, cardDue, members, existingCardLabelIds, cardLabels){

  var url = constructTrelloURL("cards/" + cardId);
  var payload = {"name": cardName,"desc": cardDesc, "due": cardDue};
  if (members !="") {
    payload.idMembers = members.replace(/\s/g,'');
  }    
  ...
  updateTrelloLabels(cardId, cardLabels, cardLabels, existingCardLabelIds);
...
function updateTrelloLabels(cardID, label,existingLabels, existingCardLabelIds){
  // if cell is empty
  if (label == "" ) {
    return;
  }  
  ...
  // loop through array
  for (var i= 0; i< labels.length;i++) {
    // get ids of existing labels
    var labelId = getIdForLabelName(labels[i],existingLabels);
    
    // if label id doesnt already exist (if new label), create label on card with new labe;
    if (labelId == null) {
      var url = constructTrelloURL("cards/"+ cardID + "/labels");
      var resp = postPayloadToTrello(url,{"color":null,"name":labels[i]});
    }
...
function getCards(cardName, cardDesc, cardPoints, listId, cardDue, cardMembers, cardLabels){  
  ...
  for (var i=values.length-1;i>=0;i--) {    
    ...
    {
      // get any existing labels on the card
      var existingCardLabelIds = values.idLabels;
      ...
      var updateCard = updateTrelloCard(cardId, cardName, cardDesc, cardDue, cardMembers, existingCardLabelIds, cardLabels);
      var cardExists = true;
    }
  }
  return cardExists;
